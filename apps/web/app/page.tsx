  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 背面（180度翻转面）：直接无缝呈现 lazzor.com 官网 */}
        <div
          style={{
            backfaceVisibility: "hidden",
            transform: "rotateY(180deg)",
          }}
          className={`absolute inset-0 w-full h-screen bg-black text-white flex flex-col overflow-hidden ${
            !isFlipped ? "pointer-events-none select-none invisible" : ""
          }`}
        >
          {/* 悬浮返回工具栏 */}
          <div className="absolute top-4 left-4 sm:left-6 z-50 flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 rounded-full bg-black/85 hover:bg-black border border-white/20 hover:border-white/40 text-white text-xs sm:text-sm font-medium transition-all backdrop-blur-md flex items-center gap-2 shadow-2xl"
            >
          ...
          {/* 直接全屏嵌入 lazzor.com 官网 */}
          <div className="w-full h-full flex-grow relative bg-black">
            {hasLoadedIframe && (
              <iframe
                src="https://lazzor.com"
                title="清迈 AI 中心官网 (lazzor.com)"
